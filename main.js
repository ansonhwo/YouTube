...
// !!!! Quality of Life Shortcuts (to be deleted) !!!!
/**
document.addEventListener('keyup', function(_test) {
  if (_test.keyCode === 192) {
    deleteChild(document.getElementById('videos'));
  }
  else if (_test.keyCode === 49) {
    var $videoList = document.getElementById('videos');

    // Wrapper div for all displayed videos
    var $videoBlock = newElement('div', 'id', 'videoblock');

    // Add elements to $videoList
    $videoList.appendChild($videoBlock);
    ...
    for (var index = 0; index < videos.length; index++) {
      ...
      $videoImage.setAttribute('src', videos[index].thumbnail);
      $videoBlock.appendChild($videoDetails);
      $videoDetails.appendChild($link);
      $link.appendChild($videoImage);
      ...
      $videoChannel.textContent = videos[index].channel;
      $videoDetails.appendChild($videoChannel);

      $videoViews.textContent = videos[index].views + " views";
      $videoDetails.appendChild($videoViews);
